"""
================================================================================
VALIDACIÓN DE CLUSTERING: Dataset Grande UdeA (No Realista)
================================================================================

Objetivo:
    Verificar si los datos etiquetados en el dataset UdeA grande (500 muestras)
    están verdaderamente agrupados según sus características. Se realiza clustering
    sin usar las etiquetas y se comparan los clusters resultantes con las
    etiquetas originales usando múltiples métricas.

Proceso:
    1. Carga el dataset preprocesado grande (sin etiquetas)
...
Características del dataset grande:
    - 500 muestras (vs 80 del realista)
    - 7 características preprocesadas
...
OUTPUT_DIR = WORKSPACE_DIR / "resultados_validacion_grande"
FIGURES_DIR = WORKSPACE_DIR / "figuras_validacion_grande"
...
def load_data() -> Tuple[pd.DataFrame, np.ndarray, List[str]]:
    ...
    Returns:
        - X: DataFrame con características preprocesadas (500, 7)
        - y: Array con etiquetas originales (500,)
        - feature_names: Nombres de las características
    """
    print("[INFO] Cargando datos del dataset GRANDE...")
    
    # Dataset preprocesado (sin etiquetas)
    processed_path = DATA_DIR / "dataset_sintetico_FIRE_UdeA_preprocesado.csv"
    X = pd.read_csv(processed_path)
    print(f"  ✓ Dataset preprocesado cargado: {X.shape}")
    
    # Dataset original (para obtener etiquetas)
    original_path = WORKSPACE_DIR / "dataset_sintetico_FIRE_UdeA.csv"
...
def run_hdbscan(X_scaled: np.ndarray, min_cluster_size: int = 10) -> Tuple[np.ndarray, int, float]:
    """
...
def plot_clustering_results(X_2d: np.ndarray, y_true: np.ndarray, y_pred: np.ndarray,
    ...
    scatter1 = axes[0].scatter(X_2d[:, 0], X_2d[:, 1], c=y_true, 
                               cmap='Set1', s=50, alpha=0.7, edgecolors='k', linewidth=0.3)
    ...
    scatter2 = axes[1].scatter(X_2d[mask, 0], X_2d[mask, 1], c=y_pred[mask], 
                               cmap='tab10', s=50, alpha=0.7, edgecolors='k', linewidth=0.3)
    
    if not mask.all():
        axes[1].scatter(X_2d[~mask, 0], X_2d[~mask, 1], c='gray', 
                       marker='X', s=100, alpha=0.5, label='Ruido')
        axes[1].legend()
...
def plot_confusion_matrices(metrics_list: List[Dict]):
    ...
    n_algorithms = len(valid_metrics)
    fig, axes = plt.subplots(1, n_algorithms, figsize=(6*n_algorithms, 5))
    
    if n_algorithms == 1:
        axes = [axes]
    
    for ax, metrics in zip(axes, valid_metrics):
...
def plot_metrics_comparison(metrics_list: List[Dict]):
    ...
    """
    valid_metrics = [m for m in metrics_list if m is not None]
    algorithms = [m['algorithm'] for m in valid_metrics]
    ari_scores = [m['ari'] for m in valid_metrics]
    nmi_scores = [m['nmi'] for m in valid_metrics]
    v_measure_scores = [m['v_measure'] for m in valid_metrics]
    purity_scores = [m['average_purity'] for m in valid_metrics]
...
def generate_report(X, y, metrics_list: List[Dict], X_umap: np.ndarray):
    ...
    csv_path = OUTPUT_DIR / "metricas_validacion_grande.csv"
    metrics_df.to_csv(csv_path, index=False)
    print(f"\n✓ Métricas guardadas en: {csv_path.name}")
    print("\n" + metrics_df.to_string())
    
    # 2. Reporte detallado en texto
    report_path = OUTPUT_DIR / "reporte_validacion_grande.txt"
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("REPORTE DE VALIDACIÓN DE CLUSTERING\n")
        f.write("Dataset UdeA Grande (No Realista) - Semana 10\n")
        f.write("="*70 + "\n\n")
        ...
        valid_metrics = [m for m in metrics_list if m is not None]
        if valid_metrics:
            best_metrics = max(valid_metrics, key=lambda x: x['average_purity'])
...
def main():
    ...
    print("\n" + "="*70)
    print("VALIDACIÓN DE CLUSTERING - DATASET UDEA GRANDE (NO REALISTA)")
    print("="*70)
    ...
    # 4a. KMeans K=2 (cantidad de clases conocida)
    print("\n" + "-"*70)
    ...
    print("-"*70)
    labels_dbscan, n_clusters_dbscan, sil_dbscan = run_dbscan(X_scaled, eps=0.4, min_samples=5)
    ...
    print("-"*70)
    labels_hdbscan, n_clusters_hdbscan, sil_hdbscan = run_hdbscan(X_scaled, min_cluster_size=15)
    ...
    print("\nArchivos generados:")
    print(f"  • metricas_validacion_grande.csv")
    print(f"  • reporte_validacion_grande.txt")
