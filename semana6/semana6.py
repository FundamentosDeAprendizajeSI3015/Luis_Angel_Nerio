...
# ==================== BALANCEO DE CLASES ====================
print("\n" + "="*70)
print("BALANCEO DE CLASES (UNDERSAMPLING)")
print("="*70)
...
print(f"\n[INFO] Clase minoritaria tiene {min_samples} muestras")
print(f"[INFO] Balanceando todas las clases a {min_samples} muestras...")

# Crear DataFrame combinado para facilitar el muestreo
data_combined = pd.concat([X.reset_index(drop=True), 
                           pd.Series(y_encoded, name='target')], axis=1)

# Submuestrear cada clase
balanced_dfs = []
for class_label in unique:
    class_data = data_combined[data_combined['target'] == class_label]
    class_sampled = class_data.sample(n=min_samples, random_state=random_state)
    balanced_dfs.append(class_sampled)
    print(f"[INFO] Clase {class_names[class_label]}: {len(class_data)} -> {len(class_sampled)} muestras")

# Combinar todas las clases balanceadas
data_balanced = pd.concat(balanced_dfs, axis=0)
...
y_encoded = data_balanced['target'].values

print(f"\n[OK] Dataset balanceado exitosamente")
print(f"[INFO] Forma del dataset balanceado: {X.shape}")
...
print("\n" + "="*70)
print("DIVISION EN ENTRENAMIENTO (60%), VALIDACIÓN (20%) Y PRUEBA (20%)")
print("="*70)

# Primer split: Separar 60% entrenamiento del 40% validación+prueba
X_train, X_temp, y_train, y_temp = train_test_split(
    X, y_encoded, test_size=0.4, random_state=random_state, stratify=y_encoded
)   

# Segundo split: Dividir el 40% en 50-50 para obtener 20% validación y 20% test
...
print("\n" + "="*70)
print("VISUALIZACIÓN DE ÁRBOLES DE DECISIÓN")
print("="*70)

# Visualizar primer árbol de Random Forest
print("\n[INFO] Graficando primer árbol de Random Forest...")
fig, ax = plt.subplots(figsize=(25, 15))
plot_tree(rf.best_estimator_.estimators_[0], 
          ...
          fontsize=10,
          ax=ax)
plt.title('Primer Árbol del Random Forest', fontsize=16, fontweight='bold', pad=20)
plt.tight_layout()
plt.savefig(os.path.join(figures_folder, 'arbol_random_forest.png'), dpi=300, bbox_inches='tight')
print("[OK] Árbol de Random Forest guardado")
plt.close()

# Visualizar primer árbol de Gradient Boosting
print("\n[INFO] Graficando primer árbol de Gradient Boosting...")
fig, ax = plt.subplots(figsize=(25, 15))
plot_tree(gb.best_estimator_.estimators_[0, 0], 
          feature_names=X.columns.tolist(),
          filled=True, 
          rounded=True,
          fontsize=10,
          ax=ax)
plt.title('Primer Árbol del Gradient Boosting', fontsize=16, fontweight='bold', pad=20)
plt.tight_layout()
plt.savefig(os.path.join(figures_folder, 'arbol_gradient_boosting.png'), dpi=300, bbox_inches='tight')
print("[OK] Árbol de Gradient Boosting guardado")
plt.close()

# Visualizar múltiples árboles de Random Forest (primeros 4)
print("\n[INFO] Graficando primeros 4 árboles de Random Forest...")
fig, axes = plt.subplots(2, 2, figsize=(30, 20))
fig.suptitle('Primeros 4 Árboles del Random Forest', fontsize=18, fontweight='bold', y=0.995)

for idx in range(4):
    ax = axes[idx // 2, idx % 2]
    plot_tree(rf.best_estimator_.estimators_[idx], 
              feature_names=X.columns.tolist(),
              class_names=class_names.tolist(),
              filled=True, 
              rounded=True,
              fontsize=8,
              ax=ax)
    ax.set_title(f'Árbol {idx + 1}', fontsize=14, fontweight='bold', pad=10)

plt.tight_layout()
plt.savefig(os.path.join(figures_folder, 'arboles_random_forest_multiples.png'), dpi=300, bbox_inches='tight')
print("[OK] Múltiples árboles de Random Forest guardados")
plt.close()
