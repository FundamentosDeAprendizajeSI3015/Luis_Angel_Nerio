"""
Agrupamiento para el dataset UdeA (semana 9), basado en el flujo del notebook
...
- DBSCAN.
- Visualizacion de todos los resultados usando UMAP 2D y 3D.
- Generacion de HTML interactivos 3D con Plotly.

Salidas:
- Graficas en carpeta "graficas/agrupamiento".
- CSV con etiquetas de cluster en carpeta "resultados/agrupamiento".
- HTML interactivos 3D en carpeta "graficas/agrupamiento".
"""
...
def ensure_output_dirs(base_dir: Path) -> Dict[str, Path]:
    ...
    figures_dir = base_dir / "graficas" / "agrupamiento"
    results_dir = base_dir / "resultados" / "agrupamiento"
...
def load_dataset(base_dir: Path) -> pd.DataFrame:
    """Carga el dataset preprocesado y valida formato numerico."""
    dataset_path = (
        base_dir
        / "datos_preprocesados"
        / "dataset_sintetico_FIRE_UdeA_preprocesado.csv"
    )
...
def save_summary(
    ...
    summary_lines = [
        "Resumen de agrupamiento - dataset UdeA",
        "",
...
def main() -> None:
    ...
    plot_umap(
        embedding=embedding,
        labels=None,
        title="UMAP - Dataset UdeA (sin clusters)",
    ...
    plot_umap_3d(
        ...
        labels=None,
        title="UMAP 3D - Dataset UdeA (sin clusters)",
    ...
    plot_umap_3d_interactive(
        embedding_3d=embedding_3d,
        labels=None,
        title="UMAP 3D - Dataset UdeA (sin clusters) - Interactivo",
    ...
    save_clustered_csv(
        ...
        dbscan_labels=dbscan.labels_,
        output_path=outputs["results"] / "dataset_con_clusters.csv",
    )
